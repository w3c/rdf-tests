# SPARQL 1.1 Protocol Requirements Document

## Version 2026-05-15

> **Scope of this revision.** Previous versions of this document
> consolidated SPARQL 1.1 and SPARQL 1.2 Protocol requirements in a
> single file. This revision retains only the requirements that apply
> to **SPARQL 1.1 Protocol** (and are inherited by SPARQL 1.2). The
> requirements that are **new in SPARQL 1.2** — primarily the version
> announcement mechanism of §2.1 — have been moved to
> [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).
>
> Earlier revisions also corrected the test-to-requirement mapping in
> Appendix B (replacing a non-existent `REQ-3.2.6` reference, among
> other errors). Those corrections are retained.

This document captures the normative requirements defined by the SPARQL 1.1 Protocol that are inherited unchanged by the SPARQL 1.2 Protocol. They were extracted from the SPARQL 1.2 Protocol Working Draft (`sparql12-protocol.html`) — the 1.2 recommendation consolidates the 1.1 requirements alongside its own additions, so every requirement in this document has a counterpart in both the original SPARQL 1.1 Protocol Recommendation and the SPARQL 1.2 Protocol recommendation.

Each requirement is derived from a MUST, SHOULD, or MAY constraint expressed in the specification (per RFC 2119) and is marked with its obligation level. Each is annotated with its source subsection in the recommendation (e.g., "Rec §2.2.1"). Where an obligation level is inferred from non-emphasized text or table constraints rather than an explicit RFC 2119 keyword in the recommendation, this is marked with a dagger (†). See the [Analysis Notes](#analysis-notes) section for details.

For the SPARQL 1.2-specific additions (version announcement mechanism, the `version` request parameter, the `version` media-type parameter on Content-Type, and the cardinality constraints on `version`), see
[`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).

## Protocol Overview

The SPARQL Protocol defines an HTTP-based means of conveying SPARQL queries and updates from clients to SPARQL processing services and returning results. The protocol consists of two operations:
...
The protocol is built on HTTP (RFC 9110 in the 1.2 reformulation; RFC 7230/7231 originally for 1.1) and is designed for compatibility with the SPARQL Query Language and SPARQL Update. A separate SPARQL Graph Store Protocol covers RESTful management of named graphs.
...
- **RDF Dataset**: A collection of a default graph and zero or more named graphs, as defined by the SPARQL Query Language.
...
> **Note:** Section 2 ("Version Announcement") of the consolidated 1.2
> requirements has been moved to
> [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).
> It contains the SPARQL 1.2-specific `version` mechanism (REQ-2.1.1
> through REQ-2.3.1), which has no counterpart in SPARQL 1.1 Protocol.
...
- **REQ-3.4.3** [MUST] (Rec §2.2.3): Clients shall set the content type header of the HTTP request to `application/sparql-query`. *(In SPARQL 1.2, this media type may additionally carry an optional `version` parameter — see [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).)*
...
- **REQ-4.3.3** [MUST] (Rec §2.3.2): Clients shall set the content type header of the HTTP request to `application/sparql-update`. *(In SPARQL 1.2, this media type may additionally carry an optional `version` parameter — see [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).)*
...
- **REQ-4.4.2** [MUST NOT]† (Rec §2.3.3): It is an error to supply the `using-graph-uri` or `using-named-graph-uri` parameters when conveying a SPARQL Update request that contains an operation using the `USING`, `USING NAMED`, or `WITH` clause.
...
- **REQ-4.4.3** [SHOULD]† (Rec §2.3.3): A SPARQL Update processor should treat each occurrence of the `using-graph-uri=g` parameter as if a `USING <g>` clause were included for every operation in the SPARQL Update request.
  > **Note:** The recommendation uses non-emphasized "should".
- **REQ-4.4.4** [SHOULD]† (Rec §2.3.3): A SPARQL Update processor should treat each occurrence of the `using-named-graph-uri=g` parameter as if a `USING NAMED <g>` clause were included for every operation in the SPARQL Update request.
...
- **REQ-8.1.1** [MUST] (Rec §5.1): A conformant SPARQL Protocol service shall implement either the `query` operation or the `update` operation (or both) in the way described in the SPARQL Protocol specification.
...
Several requirements (REQ-3.3.0, REQ-4.2.0) are derived from constraints expressed in the summary tables of §2.2 and §2.3 (e.g., "None" for query string parameters in the URL-encoded POST row) rather than from RFC 2119 keywords in prose. These tables are in normative sections and do express constraints, but the obligation levels (SHOULD NOT) are editorial interpretations.

> The 1.2-only cardinality requirements REQ-3.1.3a and REQ-4.1.3a (at-most-one `version` parameter) are also table-derived and are documented in
> [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).
...
The auto-generated conformance boilerplate in §5 of the 1.2 recommendation mentions only "The key word MAY" (singular) as an RFC 2119 keyword, despite the recommendation using MUST, MUST NOT, SHOULD, SHOULD NOT, and MAY throughout. This appears to be a ReSpec tooling artifact: the tool detected only the `<em class="rfc2119">MAY</em>` instances in §2.1 (the 1.2-specific Version Announcement section) and did not count the `<strong>` keyword instances. This does not affect the normative force of the `<strong>` keywords, which are covered by the Document Conventions in §1.1.
...
The following specifications are normatively referenced by the SPARQL Protocol (1.1 base; the 1.2 update replaces each with its corresponding 1.2 version where applicable):

- **RDF-CONCEPTS**: RDF Concepts and Abstract Syntax
- **RDF-TURTLE**: RDF Turtle
- **RDF-XML**: RDF/XML Syntax Specification
- **RFC 2119**: Key words for use in RFCs to Indicate Requirement Levels
- **RFC 3986**: Uniform Resource Identifier (URI): Generic Syntax
- **RFC 3987**: Internationalized Resource Identifiers (IRIs)
- **RFC 8174**: Ambiguity of Uppercase vs Lowercase in RFC 2119 Key Words
- **HTTP Semantics**: RFC 7230/7231 in SPARQL 1.1; RFC 9110 in SPARQL 1.2
- **SPARQL-GRAPH-STORE-PROTOCOL**: SPARQL Graph Store Protocol
- **SPARQL-QUERY**: SPARQL Query Language
- **SPARQL-RESULTS-CSV-TSV**: SPARQL Query Results CSV and TSV Formats
- **SPARQL-RESULTS-JSON**: SPARQL Query Results JSON Format
- **SPARQL-RESULTS-XML**: SPARQL Query Results XML Format
- **SPARQL-UPDATE**: SPARQL Update
- **UTR36**: Unicode Security Considerations

See [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md) for the SPARQL 1.2 versions of these references.

## Relationship to SPARQL 1.2 Protocol

A conformant SPARQL 1.2 Protocol service must satisfy:

- the requirements in this document (the 1.1 base, inherited unchanged); and
- the additional 1.2-specific requirements in
  [`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).

The 1.2 additions are:

- the version announcement mechanism (`version` syntactical directive,
  `version` request parameter, and `version` media-type parameter)
  in Rec §2.1;
- the corresponding query/update parameter slots (REQ-3.1.3 / REQ-3.1.3a,
  REQ-4.1.3 / REQ-4.1.3a); and
- updated normative references (RFC 9110 in place of RFC 7230/7231,
  SPARQL 1.2 query/update/results specifications in place of their
  1.1 counterparts).

## Requirements Summary

This document contains the **SPARQL 1.1** subset (inherited unchanged by SPARQL 1.2):

| Category | MUST | SHOULD | MAY | MUST NOT | SHOULD NOT | Total |
|---|---|---|---|---|---|---|
| HTTP Foundation | 2 | 0 | 0 | 0 | 0 | 2 |
| Query Operation | 14 | 4 | 14 | 1 | 1 | 34 |
| Update Operation | 8 | 5 | 11 | 1 | 1 | 26 |
...
| **Total (1.1 base)** | **32** | **10** | **33** | **2** | **2** | **79** |

The 10 additional **1.2-specific** requirements (Section 2 — Version
Announcement, plus REQ-3.1.3/REQ-3.1.3a/REQ-4.1.3/REQ-4.1.3a) are
documented in
[`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).
Together the two documents cover all 89 normative requirements derived
from the consolidated 1.2 recommendation.

Of the 79 requirements in this document, 21 (marked †) have obligation
levels inferred from non-emphasized text or table constraints rather
than explicit RFC 2119 keywords in the recommendation.

## Appendix B: Test-to-Requirement Mapping

This appendix provides the mapping between SPARQL 1.1 Protocol test names (as defined in [`manifest.ttl`](manifest.ttl) in this directory) and the requirements they validate. The mapping below covers only the 1.1 tests; the 1.2-specific tests and their requirement mapping are in
[`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).

Earlier revisions of this document corrected the `rdfs:comment` references in the manifest, which had used an incorrect numbering scheme from earlier drafts. The corrected mapping is below.
...
| update_post_direct | REQ-4.1.2, REQ-4.1.3 | **REQ-4.3.1, REQ-4.3.3** | Tests direct POST update. REQ-4.3.1 permits it, REQ-4.3.3 requires Content-Type. REQ-4.1.2 is about one update; REQ-4.1.3 (in older drafts) was about version. |
| query_base_uri | REQ-5.1.1 | **REQ-5.1.1** | Tests BASE keyword resolution. Correct. |

### B.2 Summary of Manifest Errors

Of the original 45 tests (across both 1.1 and 1.2), **27 had incorrect requirement references** in the manifest `rdfs:comment`. New tests added for missing coverage: `bad_update_form_body_missing` (REQ-4.2.2), `query_post_form_dataset` (REQ-3.5.1), and `bad_multiple_queries_form` (REQ-3.1.2). The original errors fell into these categories:
...
| REQ-3.1.2 | bad_multiple_queries, bad_multiple_queries_form |
| REQ-3.2.1 | query_get |
...
| REQ-3.4.3 | query_post_direct, bad_query_missing_direct_type |
| REQ-3.4.5 | bad_query_non_utf8 |
| REQ-3.5.1 | query_dataset_default_graph, query_dataset_default_graphs_get, query_dataset_default_graphs_post, query_dataset_named_graphs_get, query_dataset_named_graphs_post, query_dataset_full, query_post_form_dataset |
...
For the 1.2-specific requirements (REQ-2.1.*, REQ-3.1.3, REQ-3.1.3a,
REQ-4.1.3, REQ-4.1.3a) and the tests that exercise them, see
[`../../sparql12/protocol/requirements.md`](../../sparql12/protocol/requirements.md).
